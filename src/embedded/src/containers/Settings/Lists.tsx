...
// import type { ListPropTypes, SvgIconProps } from "@/embedded/ui";
...
const getThemeIcon = (theme: ThemeMode) => {
  const iconProps = { color: "gray", size: 16 };
...
export function SettingsLists() {
    ...
    title: (
      ...
      <ThemeSection>
        <Tabs
          ...
          items={themesList}
          value={theme}
          onChange={(item: { id: string }) => setTheme(item.id as ThemeMode)}
          type="primary"
          borderRadius="small"
        />
      </ThemeSection>
  ...
  const settingItems: any = isLiquidityHidden ? [] : [bridgeItem, exchangeItem];
