  ...
  const completeModalDesc =
    swap.status === "success" ? (
      `You have received ${numberToString(
        ...
        TOKEN_AMOUNT_MIN_DECIMALS,
        TOKEN_AMOUNT_MAX_DECIMALS,
      )} ${steps[numberOfSteps - 1].to.token.displayName} in ${getConciseAddress(
        swap.wallets[steps[numberOfSteps - 1].to.chain.displayName]?.address ||
          "",
      ...
      <ErrorMessages>
        <Typography
          ...
          align="center"
        >
          {!stepDetailMessage ? "Transaction was not sent." : ""}
    ...
    >
      <PageContainer compact view>
        <HeaderDetails>
              ...
              <IconButton
                ...
                onClick={() => {
                  if (typeof handleCopy === "function")
                    handleCopy.bind(null, requestId || "");
                }}
              >
          ...
          <div className={rowStyles()}>
            <Typography variant="label" size="large" color="neutral700">
              {`${swap.finishTime ? "Finished at" : "Created at"}:`}
            </Typography>
            <Typography variant="label" size="small" color="neutral700">
              {swapDate}
            </Typography>
